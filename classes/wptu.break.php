...
class WPTicketUltraBreak
{
	...
	function __construct() 
	{
		...
		add_action( 'wp_ajax_'.$this->ajax_p.'_get_break_add',  array( &$this, 'get_break_add_frm' ));
		add_action( 'wp_ajax_'.$this->ajax_p.'_break_add_confirm',  array( &$this, 'break_add_confirm' ));
		add_action( 'wp_ajax_'.$this->ajax_p.'_get_current_staff_breaks',  array( &$this, 'get_current_staff_breaks' ));
		add_action( 'wp_ajax_'.$this->ajax_p.'_delete_break',  array( &$this, 'delete_break' ));	
	...
	public function ini_module()
	{
		global $wpdb;	
			...
			$query = 'CREATE TABLE IF NOT EXISTS ' . $wpdb->prefix . ''.$this->table_prfix.'_staff_availability_breaks (
	...
	public function break_add_confirm()
	{
		...
		{				
			...
			$html = __('ERROR. Duplicated Break!', 'wp-ticket-ultra')	;
	...
	public function get_break_add_frm($staff_id = null, $day_id = null )
	{
		...
		$html .=''.$this->get_breaks_drop_downs($day_id,'bup-break-from-'.$day_id ,'bup_select_start', $staff_id). '<span> '.__('to', 'wp-ticket-ultra').' </span>' .$this->get_breaks_drop_downs($day_id,'bup-break-to-'.$day_id ,'bup_select_end', $staff_id).'';
		
		$html .= '<button name="bup-btn-add-break-confirm" id="bup-btn-add-break-confirm" class="bup-button-submit-breaks" day-id="'.$day_id.'">'.__('Add','wp-ticket-ultra').'	</button>';
	...
	public function get_breaks_drop_downs($day, $cbox_id, $select_start_to_class, $staff_id)
	{
		...
		//$html .= '<option value="" '.$selected.'>'.__('OFF','wp-ticket-ultra').'</option>';
	...
	public function get_staff_breaks($staff_id)
	{
		...
		$html .='<div class="bup-break-details-header" widget-id="1"><h3> '.__('Monday','wp-ticket-ultra').'<h3>';
				
		$html .='<span class="bup-breaks-add" id="bup-widgets-icon-close-open-id-1"  day-id="1" >'.__('Add Break','wp-ticket-ultra').'</span>';
		...
		$html .='<div class="bup-break-details-header" widget-id="1"><h3> '.__('Tuesday','wp-ticket-ultra').'<h3>';
				
		$html .='<span class="bup-breaks-add"  day-id="2" >'.__('Add Break','wp-ticket-ultra').'</span>';
		...
		$html .='<div class="bup-break-details-header" widget-id="1"><h3> '.__('Wednesday','wp-ticket-ultra').'<h3>';
				
		$html .='<span class="bup-breaks-add"  day-id="3" >'.__('Add Break','wp-ticket-ultra').'</span>';
		...
		$html .='<div class="bup-break-details-header" widget-id="1"><h3> '.__('Thursday ','wp-ticket-ultra').'<h3>';
				
		$html .='<span class="bup-breaks-add"  day-id="4" >'.__('Add Break','wp-ticket-ultra').'</span>';
		...
		$html .='<div class="bup-break-details-header" widget-id="1"><h3> '.__('Friday','wp-ticket-ultra').'<h3>';
				
		$html .='<span class="bup-breaks-add" day-id="5" >'.__('Add Break','wp-ticket-ultra').'</span>';
		...
		$html .='<div class="bup-break-details-header" widget-id="1"><h3> '.__('Saturday','wp-ticket-ultra').'<h3>';
				
		$html .='<span class="bup-breaks-add"  day-id="6" >'.__('Add Break','wp-ticket-ultra').'</span>';
		...
		$html .='<div class="bup-break-details-header" widget-id="1"><h3> '.__('Sunday','wp-ticket-ultra').'<h3>';
				
		$html .='<span class="bup-breaks-add" day-id="7" >'.__('Add Break','wp-ticket-ultra').'</span>';
	...
	public function get_current_staff_breaks($staff_id = null, $day_id = null)
	{
		...
		if ( !empty( $results ))
		{
			...
			foreach ( $results as $row )
			{
				...
				$html .= '<span class="bup-breaks-remove" id="bup-break-add-'.$day_id.'"><a href="#" class="ubp-break-delete-btn" title="'.__("Delete Break", 'wp-ticket-ultra').'" break-id="'.$row->break_id.'" day-id="'.$day_id.'"><i class="fa fa-remove"></i></a></span>';
				$html .= '</li>';
			...
			$html = __("There are no breaks for this day.", 'wp-ticket-ultra')	;			
...
$key = "breaks";
$this->{$key} = new WPTicketUltraBreak();
?>